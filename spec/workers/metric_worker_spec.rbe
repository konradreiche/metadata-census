...
describe MetricWorker do

  describe '.symbolize_keys' do

    context 'applied on a simple hash' do
      grades = { "Jane Doe" => 10, "Jim Doe" => 6 }
      subject { MetricWorker.symbolize_keys(grades) }
    ...
    context 'applied on a nested hash' do
      ...
      subject { MetricWorker.symbolize_keys(document)[:options] }
    ...
    context 'applied on a hash with an array' do
      ...
      subject { MetricWorker.symbolize_keys(book)[:authors] }
